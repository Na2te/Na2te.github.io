...
다만 <mark>웹 링크를 소유권을 확인하게 되면, 이를 해당 링크의 다루는 Handler로 앱을 지정할 수 있고, 이를 바탕으로 앱의 특정 화면으로 이동하기 위한 딥링크를 만들 수 있게 된다</mark>
  ...
  + ### 실행 결과
    <video width="300" src="/assets/img/study/android/Deeplink/딥링크 시연.mp4" style="display:block; margin-left: auto; margin-right: auto;" alt="뚝딱" controls></video>앱 최초 진입 화면인 스터디 그룹 리스트 목록이 아니라 바로 특정 그룹 화면으로 이동하는 것을 볼 수 있다
    {:.figcaption}
